...
object NotificationHelper {
    ...
    fun sendSmsNotification(
        ...
        val builder =
            NotificationCompat.Builder(context, SMS_CHANNEL_ID)
        ...
        val amountText =
            amountMatch?.groups?.get(1)?.value?.let { amount ->
                try {
        ...
        val notThisSMSIntent =
            Intent(context, NotificationActionReceiver::class.java).also {
                it.action = NotificationActionReceiver.ACTION_NOT_THIS_SMS
                it.putExtra("sms_id", smsId)
            }
        val notThisSMPendingIntent =
